...
### Canonical initial-state generation

Phase 0B-R1.2 does not attempt to physically rearrange a random cable into
the benchmark start state. Instead, the cable and its point-to-point
constraints are created directly in the canonical pre-insertion geometry.

This is benchmark initialization, not an evaluated manipulation action.
The fixture is subsequently created, a common PyBullet snapshot is saved,
and no object teleportation or constraint reconstruction is allowed in any
evaluated branch.
