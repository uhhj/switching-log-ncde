...
def capture_world_state(env, task):
    bead_pose = [
        p.getBasePositionAndOrientation(int(x)) for x in task.cable_bead_IDs
    ]
    ...
    return {
        ...
        "bead_linear_velocities": np.asarray(
            [x[0] for x in bead_vel], dtype=np.float64
        ),
...
def restore_joint_control_state(env, world_state):
    """Hold restored joints so a previous branch's motor target cannot leak."""
    ...
    p.setJointMotorControlArray(
        ...
        positionGains=np.ones(len(env.joints), dtype=np.float64),
    )
